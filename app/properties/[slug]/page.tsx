...
interface Dealer {
  _id: string; agencyName: string; whatsapp: string; city: string;
  bio: string; logo: string; isVerified: boolean;
  areasServed: string[]; experience: number;
}
interface Property {
  _id: string; title: string; description: string;
  purpose: string; type: string; price: number;
  ...
  images: Array<string | { url: string; publicId?: string }>; amenities: string[];
...
export default function PropertyDetailPage() {
  ...
  useEffect(() => {
    fetch(`${API}/properties/${slug}`)
      .then(r => r.json())
      .then(j => { if (j.success) setProperty(j.data); })
      .catch(() => {})
  ...
  const sendInquiry = async (e: React.FormEvent) => {
    ...
    try {
      const r = await fetch(`${API}/inquiries`, {
        ...
        body: JSON.stringify({ ...inquiry, propertyId: property?._id }),
      });
