...
interface Property {
  _id: string; slug: string; title: string; purpose: string; type: string;
  price: number; area: number; areaUnit: string;
  bedrooms?: number; bathrooms?: number;
  city: string; areaName: string;
  images: Array<string | { url: string; publicId?: string }>; isFeatured: boolean;
...
function PropertiesContent() {
  ...
  const fetch_ = useCallback(async () => {
    setLoading(true);
    try {
      const r = await fetch(`${API}/properties?${buildQuery()}`);
  ...
  const activeTags = [
    ...
    minPrice    && { label: `Min ${formatPrice(Number(minPrice))}`, clear: () => clearTag(setMinPrice) },
    maxPrice    && { label: `Max ${formatPrice(Number(maxPrice))}`, clear: () => clearTag(setMaxPrice) },
          ...
          <div className="max-w-7xl mx-auto px-4 pb-2 flex gap-2 flex-wrap">
            {activeTags.map(t => (
              <span key={t.label} className="flex items-center gap-1 bg-green-50 text-green-700 border border-green-200 rounded-full px-3 py-0.5 text-xs font-medium">
                {t.label}
                <button onClick={t.clear}><X size={11}/></button>
              </span>
            ))}
            <button onClick={() => { setPurpose("SALE"); setCity(""); setAreaName(""); setType(""); setMinPrice(""); setMaxPrice(""); setMinArea(""); setBedrooms(""); setBathrooms(""); setFurnishing(""); setAddedWithin(""); setPage(1); }}
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5 space-y-4">
            ...
            <div>
              ...
              <div className="flex gap-1.5 flex-wrap">
                {["Any",...BEDS].map(b => (
                  <button key={b} onClick={() => setBedrooms(b==="Any" ? "" : b)}
            ...
            <div>
              ...
              <div className="flex gap-1.5 flex-wrap">
                {["Any","1","2","3","4","5"].map(b => (
                  <button key={b} onClick={() => setBathrooms(b==="Any" ? "" : b)}
                  ...
                  <div className="flex-1 p-5 flex flex-col justify-between">
                    <div>
                      <div className="flex items-start justify-between gap-2">
                        ...
                        <button className="text-gray-300 hover:text-red-400 transition-colors flex-shrink-0 mt-0.5" onClick={e => e.preventDefault()}>
                          <Heart size={18}/>
                        </button>
                      </div>
...
export default function PropertiesPage() {
  return (
    <Suspense fallback={<div className="min-h-screen flex items-center justify-center"><div className="w-10 h-10 border-2 border-green-500 border-t-transparent rounded-full animate-spin"/></div>}>
      <PropertiesContent />
    </Suspense>
  );
}
