...
interface Project {
  ...
  developer?: string; minPrice?: number; maxPrice?: number;
  images: string[]; isFeatured: boolean; views: number;
  offering: string[]; status: string; createdAt: string;
}
...
export default function NewProjectsPage() {
  ...
  useEffect(() => {
    setLoading(true);
    fetch(`${API}/projects?page=${page}&limit=20`)
      .then(r => r.json())
      .then(j => { setProjects(j.data || []); setTotal(j.total || 0); setPages(j.pages || 1); })
      .catch(() => setProjects([]))
      ...
      <div className="bg-white border-b border-gray-100 shadow-sm sticky top-0 z-20">
        <div className="max-w-7xl mx-auto px-4 py-4 flex items-center gap-4">
          <Link href="/" className="flex items-center gap-2">
            ...
            <span className="text-base font-bold text-gray-800">arzepak<span className="text-green-600">Properties</span></span>
          </Link> 
            ...
            {projects.map(p => (
              <Link key={p._id} href={`/new-projects/${p.slug}`}
                ...
                <div className="relative h-52 overflow-hidden">
                  <img src={p.images?.[0] || "https://images.unsplash.com/photo-1486325212027-8081e485255e?w=600"}
                  ...
                  <span className={`absolute top-3 right-3 text-[10px] font-bold px-2.5 py-1 rounded-full ${
                    p.status === "BOOKING_OPEN"       ? "bg-green-600 text-white"  :
                    p.status === "UNDER_CONSTRUCTION" ? "bg-orange-500 text-white" :
                    p.status === "LAUNCHING_SOON"     ? "bg-purple-600 text-white" :
                    p.status === "COMPLETED"          ? "bg-blue-600 text-white"   :
                    p.status === "SOLD_OUT"           ? "bg-red-600 text-white"    :
                ...
                <div className="p-5">
                  ...
                  <div className="flex items-center gap-1 text-gray-500 text-xs mb-3">
                    <MapPin size={12} className="text-green-500 flex-shrink-0" />{p.address}
                  </div>
                  ...
                  {p.offering?.length > 0 && (
                    <div className="flex gap-1.5 flex-wrap mb-3">
                      {p.offering.map(o => (
                        <span key={o} className="text-[10px] font-semibold bg-gray-100 text-gray-600 px-2 py-0.5 rounded-full">{o}</span>
                      ))}
                    </div>
                  )}
                    ...
                    <div>
                      {p.minPrice && p.maxPrice ? (
                        ...
                        <>
                          ...
                          <div className="font-bold text-green-700 text-sm">PKR {formatPrice(p.minPrice)}</div>
                        </>
                      ) : null}
                    </div>
                    <div className="text-right">
                      {p.developer && <div className="flex items-center gap-1 text-xs text-gray-400 justify-end"><Building2 size={11}/>{p.developer}</div>}
                      <div className="flex items-center gap-1 text-xs text-gray-400 justify-end mt-0.5"><Eye size={11}/>{p.views}</div>
                    </div>
                  </div>
                </div>
              </Link>
        ...
        {pages > 1 && (
          <div className="flex justify-center gap-2 mt-8">
            <button disabled={page === 1} onClick={() => setPage(p => p - 1)}
              className="px-5 py-2 border border-gray-200 rounded-xl text-sm disabled:opacity-40 hover:bg-gray-50">← Prev</button>
            <span className="text-sm text-gray-500 self-center">Page {page} of {pages}</span>
            <button disabled={page === pages} onClick={() => setPage(p => p + 1)}
              className="px-5 py-2 border border-gray-200 rounded-xl text-sm disabled:opacity-40 hover:bg-gray-50">Next →</button>
          </div>
        )}
      </div>
    </div>
